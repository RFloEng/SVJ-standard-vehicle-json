...
| Area | Change |
|---|---|
| Viewer v4.2 | `svj_viewer_v4.1.html` → `svj_viewer_v4.2.html`. New **Performance** tab: straight-line full-throttle model (engine `torque_curve` or `electric.motors`, gearbox ratios and shift time, final drive, transfer-case range, hub reduction, torque-converter multiplication for automatics, traction limit μ × driven-axle load with longitudinal transfer and aero lift, drag, rolling resistance, grade, wheel/engine inertia). Charts: speed vs time with gear changes, engine rpm vs speed (per-gear lines + simulated sawtooth), tractive force per gear vs road load; hover readouts. KPIs 0–100 / 0–50 (trucks) / 0–200, 400 m, top speed (drag- or rev-limited), gradeability, power-to-weight, compared with `benchmarks` (`accel_0_100kph`, `top_speed`, `max_speed`, `geared_road_speed`, gradeability). Controls: load (kerb / kerb + driver / GVM), transfer range, μ, grade, shift and launch rpm. Inputs table tags each value as from the file or a default. Old speed/rpm charts removed from Powertrain (they used FL tyre radius only and stopped early); Powertrain final drive now skips inter-axle diffs |
